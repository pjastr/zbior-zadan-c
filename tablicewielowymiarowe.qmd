...
18. Napisz funkcję `flipDiag`, której argumentem są dwie dodatnie liczby całkowite $n$ i $m$ oraz dwuwymiarowa tablica elementów (zawierająca zmienne typu int) wymiaru $n\times m$. Funkcja ma zamienić kolejność elementów leżących na głównej przekątnej. Funkcja powinna działać tylko dla macierzy kwadratowych, w przeciwnym przypadku nie powinna nic robić. Stwórz przypadek testowy.
