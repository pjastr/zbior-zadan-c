...
W tym dziale samo słowa tablica (lub tablica elementów) dotyczy notacji definiowania tablic przez "nawiasy kwadratowe". W przypadku określenia tablica tablic - powinny być zadeklarowane jako podwójny wskaźnik.
...
30. Napisz funkcję `rotateRight`, która dostaje jako argumenty dwie dodatnie liczby całkowite `n` i `m` oraz dwuwymiarową tablicę tablic o elementach typu `int` o wymiarach $n\times m$. Funkcja ma zwrócić wskaźnik na nowo-utworzoną dynamiczną dwuwymiarową tablicę o wymiarach $m\times n$ powstałą z przekazanego argumentu poprzez obrót o 90 stopni w prawo (zgodnie z ruchem wskazówek zegara). Stwórz przypadek testowy dla funkcji.
...
31. Napisz funkcję `mulMat`, która dostaje jako argumenty trzy dodatnie liczby całkowite `n`, `m` i `p` oraz dwie dwuwymiarowe tablice tablic o elementach typu `int`: pierwszą o wymiarach $n\times m$ oraz drugą o wymiarach $m\times p$. Funkcja ma zwrócić wskaźnik na nowo-utworzoną dynamiczną dwuwymiarową tablicę o wymiarach $n\times p$ będącą iloczynem macierzowym obu tablic. Stwórz przypadek testowy dla funkcji.
...
32. Napisz funkcję `subMat`, która dostaje jako argumenty dwie dodatnie liczby całkowite `n` i `m`, dwuwymiarową tablicę tablic o elementach typu `int` o wymiarach $n\times m$ oraz dwie nieujemne liczby całkowite `r` i `c` ($0\leq r<n$, $0\leq c<m$). Funkcja ma zwrócić wskaźnik na nowo-utworzoną dynamiczną dwuwymiarową tablicę o wymiarach $(n-1)\times(m-1)$ powstałą z przekazanej tablicy poprzez usunięcie wiersza o indeksie `r` oraz kolumny o indeksie `c`. Stwórz przypadek testowy dla funkcji.

Przykład (dla $r=1$, $c=2$):
...
33. Napisz funkcję `mergeHoriz`, która dostaje jako argumenty trzy dodatnie liczby całkowite `n`, `m1` i `m2` oraz dwie dwuwymiarowe tablice tablic o elementach typu `int`: pierwszą o wymiarach $n\times m_1$ oraz drugą o wymiarach $n\times m_2$. Funkcja ma zwrócić wskaźnik na nowo-utworzoną dynamiczną dwuwymiarową tablicę o wymiarach $n\times (m_1+m_2)$ powstałą poprzez sklejenie obu tablic poziomo (druga tablica dostawiona z prawej strony pierwszej). Stwórz przypadek testowy dla funkcji.
...
34. Napisz funkcję `borderFrame`, która dostaje jako argumenty dwie dodatnie liczby całkowite `n` i `m` oraz dwuwymiarową tablicę tablic o elementach typu `int` o wymiarach $n\times m$. Funkcja ma zwrócić wskaźnik na nowo-utworzoną dynamiczną dwuwymiarową tablicę o wymiarach $(n+2)\times(m+2)$ powstałą poprzez otoczenie przekazanej tablicy ramką z zer. Stwórz przypadek testowy dla funkcji.
